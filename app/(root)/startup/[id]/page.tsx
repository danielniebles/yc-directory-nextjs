...
export default async function Startup({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <section className="section_container">
      <img src={post.image} alt="thumbnail" className="w-full h-[300px] object-cover rounded-xl" />
      <div className="space-y-5 mt-10 max-w-4xl mx-auto">
        <div className="flex-between gap-5">
          <Link href={`/user/${post.author?.id}`} className="flex gap-2 items-center mb-3">
            <Image src={post.author?.image as string} alt="avatar" width={64} height={64} className="rounded-full drop-shadow-lg" />
            <div>
